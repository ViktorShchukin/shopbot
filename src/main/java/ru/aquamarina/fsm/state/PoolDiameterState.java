...
public class PoolDiameterState implements FsmState {
    public static final String NAME = "PoolDiameter";

    private final Logger log = LoggerFactory.getLogger(IndexState.class);

    private final User user;

    public PoolDiameterState(User user) {
        this.user = user;
    }
    ...
    public Result<FsmState, Error> doWork(FsmContextHolder context, Command command) {
        return switch (command) {
            case UserInputCmd input -> context.getPoolInfoService()
                    .update(user.getId(), null, null, null, 1L, null)
                    .mapValue(res -> new GuideState(user));
    ...
    public Form getForm(FsmContextHolder context) {
        return new PoolDiameterForm(user);
    }
