...
public class ContactListOfChemicalsState implements FsmState {

    public static final String NAME = "ContactListOfChemicals";

    private final Logger log = LoggerFactory.getLogger(IndexState.class);

    private final User user;

    public ContactListOfChemicalsState(User user) {
        this.user = user;
    }
    ...
    public Result<FsmState, Error> doWork(FsmContextHolder context, Command command) {
        return switch (command) {
            case BackCmd cmd -> Result.ok(new ListOfChemicalsState(user));
